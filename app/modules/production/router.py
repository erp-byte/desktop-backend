...
async def create_plan_with_ai(request: Request, body: CreatePlanWithAIRequest):
    ...
    async with pool.acquire() as conn:
        context = await collect_planning_context(conn, body.entity, target_date, fulfillment_ids, user_constraints=user_constraints)
...
async def list_job_cards_all(
    ...
    result = await list_job_cards(request, entity, status, team_leader, floor, factory, stage, search, customer, article, date_from, date_to, page=1, page_size=100000)
    return result["results"]
...
async def team_dashboard(
    ...
    async with pool.acquire() as conn:
        ...
        if date_to:
            conditions.append(f"created_at::date <= ${idx}::date"); params.append(date_to); idx += 1
        conditions.append("status NOT IN ('closed', 'completed')")
...
class ManualAckRequest(BaseModel):
    indent_lines: list[dict]  # [{ indent_type, indent_id }] — empty list = acknowledge all
    acknowledged_by: str
