...
async def approve_plan(request: Request, plan_id: int):
    """Approve a production plan and create production orders."""
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
        ...
        # Auto-create production orders from approved plan
        from app.modules.production.services.job_card_engine import create_production_orders
        async with conn.transaction():
            await conn.execute(
                "UPDATE production_plan SET status = 'approved' WHERE plan_id = $1", plan_id,
            )
            result = await create_production_orders(conn, plan_id, plan['entity'])

    if "error" in result:
        raise HTTPException(status_code=400, detail=result.get("message", result["error"]))
    return {"plan_id": plan_id, "status": "approved", "orders_created": result.get("orders_created", 0), "orders": result.get("orders", [])}
...
async def acknowledge_indent(request: Request, indent_id: int, body: IndentAcknowledgeRequest):
    """Acknowledge a purchase indent."""
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
        indent = await conn.fetchrow("SELECT status FROM purchase_indent WHERE indent_id = $1", indent_id)
        if not indent:
            raise HTTPException(status_code=404, detail="Indent not found")
        if indent['status'] not in ('raised', 'draft'):
            raise HTTPException(status_code=400, detail=f"Cannot acknowledge indent with status '{indent['status']}'")
        await conn.execute(
            "UPDATE purchase_indent SET status = 'acknowledged', acknowledged_by = $2, acknowledged_at = NOW() WHERE indent_id = $1",
            indent_id, body.acknowledged_by,
        )
    return {"indent_id": indent_id, "status": "acknowledged"}


@router.put("/indents/{indent_id}/link-po")
async def link_indent_po(request: Request, indent_id: int, body: IndentLinkPORequest):
    """Link a purchase indent to a PO reference."""
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
        indent = await conn.fetchrow("SELECT status FROM purchase_indent WHERE indent_id = $1", indent_id)
        if not indent:
        ...
        await conn.execute(
            ...
            indent_id, body.po_reference,
        )
    return {"indent_id": indent_id, "status": "po_created", "po_reference": body.po_reference}
...
async def mark_all_alerts_read(request: Request, entity: str = Query(None)):
    """Mark all alerts as read, optionally filtered by entity."""
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
        if entity:
            result = await conn.execute(
                "UPDATE store_alert SET is_read = TRUE WHERE is_read = FALSE AND entity = $1", entity,
            )
        else:
            result = await conn.execute("UPDATE store_alert SET is_read = TRUE WHERE is_read = FALSE")
...
@router.get("/store/pending-materials")
async def store_pending_materials(
    request: Request,
    entity: str = Query(None),
    status: str = Query(None),
    search: str = Query(None),
    ...
    """List purchase indents raised from job cards — store team view.
    Only shows indents that have a job_card_id (raised by floor manager from a job card)."""
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
        conditions = ["pi.job_card_id IS NOT NULL"]
        params = []
        idx = 1

        if entity:
            conditions.append(f"pi.entity = ${idx}")
            params.append(entity)
            idx += 1
        if status:
            statuses = [s.strip() for s in status.split(',')]
            ph = ', '.join(f'${idx + j}' for j in range(len(statuses)))
            conditions.append(f"pi.status IN ({ph})")
            params.extend(statuses)
            idx += len(statuses)
        if search:
            conditions.append(f"(pi.material_sku_name ILIKE ${idx} OR pi.indent_number ILIKE ${idx} OR jc.job_card_number ILIKE ${idx})")
            params.append(f"%{search}%")
            idx += 1
        ...
        total = await conn.fetchval(f"""
            SELECT COUNT(*) FROM purchase_indent pi
            LEFT JOIN job_card jc ON jc.job_card_id = pi.job_card_id
            WHERE {where}
        """, *params)

        rows = await conn.fetch(f"""
            SELECT pi.indent_id, pi.indent_number, pi.material_sku_name,
                   pi.required_qty_kg, pi.status, pi.entity, pi.indent_source,
                   pi.cascade_reason, pi.customer_name, pi.created_at,
                   pi.job_card_id,
                   jc.job_card_number, jc.fg_sku_name, jc.floor,
                   jc.assigned_to_team_leader, jc.batch_number
        ...
        for r in rows:
            d = dict(r)
            d['required_qty_kg'] = float(d['required_qty_kg']) if d['required_qty_kg'] else 0
...
class AllocateIndentBody(BaseModel):
    allocated_qty_kg: float
    allocated_by: str


@router.put("/store/indents/{indent_id}/allocate")
async def allocate_indent(request: Request, indent_id: int, body: AllocateIndentBody,
                           user=Depends(require_permission("production", "inventory", action="edit"))):
    """Store team allocates material to an indent — marks it as acknowledged with allocated qty."""
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
        indent = await conn.fetchrow("SELECT * FROM purchase_indent WHERE indent_id = $1", indent_id)
        if not indent:
            raise HTTPException(status_code=404, detail="Indent not found")
        if indent['status'] not in ('raised', 'insufficient_stock'):
            raise HTTPException(status_code=400, detail=f"Cannot allocate indent in status '{indent['status']}'")

        await conn.execute("""
            UPDATE purchase_indent SET
                status = 'acknowledged',
                allocated_qty_kg = $2,
                allocated_by = $3,
                allocated_at = NOW(),
            ...
            WHERE indent_id = $1
        """, indent_id, body.allocated_qty_kg, body.allocated_by)

        # Also update the job card indent line issued_qty if linked
        jc_id = indent['job_card_id']
        if jc_id:
            material = indent['material_sku_name']
            # Try RM first, then PM
            updated = await conn.execute("""
                UPDATE job_card_rm_indent SET
                    issued_qty = COALESCE(issued_qty, 0) + $3,
                    status = CASE WHEN (COALESCE(issued_qty, 0) + $3) >= gross_qty THEN 'fulfilled' ELSE 'partial' END,
                    manual_ack_by = $4, manual_ack_at = NOW()
                WHERE job_card_id = $1 AND material_sku_name ILIKE $2 AND status != 'fulfilled'
            """, jc_id, f"%{material}%", body.allocated_qty_kg, body.allocated_by)
            if updated == "UPDATE 0":
                await conn.execute("""
            ...
            # Check if all indents fulfilled → auto-transition job card
            rm_pending = await conn.fetchval(
                "SELECT COUNT(*) FROM job_card_rm_indent WHERE job_card_id = $1 AND status != 'fulfilled'", jc_id)
            pm_pending = await conn.fetchval(
                "SELECT COUNT(*) FROM job_card_pm_indent WHERE job_card_id = $1 AND status != 'fulfilled'", jc_id)
            if rm_pending == 0 and pm_pending == 0:
                await conn.execute(
                    "UPDATE job_card SET status = 'material_received' WHERE job_card_id = $1 AND status NOT IN ('in_progress','completed','closed')",
                    jc_id)

    return {"indent_id": indent_id, "status": "acknowledged", "allocated_qty_kg": body.allocated_qty_kg}


class InsufficientStockBody(BaseModel):
    reason: str
    marked_by: str


@router.put("/store/indents/{indent_id}/insufficient")
async def mark_insufficient(request: Request, indent_id: int, body: InsufficientStockBody):
    """Store team marks an indent as insufficient stock — keeps it open for later allocation."""
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
        indent = await conn.fetchrow("SELECT * FROM purchase_indent WHERE indent_id = $1", indent_id)
        if not indent:
            raise HTTPException(status_code=404, detail="Indent not found")
        if indent['status'] not in ('raised',):
            raise HTTPException(status_code=400, detail=f"Cannot mark insufficient for status '{indent['status']}'")

        await conn.execute("""
            UPDATE purchase_indent SET
                status = 'insufficient_stock',
                insufficient_reason = $2,
                acknowledged_by = $3,
                acknowledged_at = NOW()
            WHERE indent_id = $1
        """, indent_id, body.reason, body.marked_by)

    return {"indent_id": indent_id, "status": "insufficient_stock"}
...
@router.get("/inventory/batches/{batch_id}/history")
async def get_batch_history_endpoint(request: Request, batch_id: str):
    from app.modules.production.services.inventory_service import get_batch, get_batch_history
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
        batch = await get_batch(conn, batch_id)
...
async def export_job_card_pdf(
    request: Request,
    job_card_id: int,
    mode: str = Query("full", regex="^(full|bom)$"),
):
    """Export job card as PDF. mode=full (with actuals) or bom (empty for paperwork)."""
    from app.modules.production.services.job_card_engine import get_job_card_detail
    ...
    # BOM mode allowed for in_progress, completed, closed
    # Full mode only for completed or closed
    status = jc_data.get('status', '')
    if mode == 'full' and status not in ('completed', 'closed'):
        raise HTTPException(status_code=400, detail="Full PDF only available for completed/closed job cards")

    pdf_bytes = generate_job_card_pdf(jc_data, mode=mode)
    filename = f"{jc_data.get('job_card_number', 'JC')}.pdf"
    return Response(
        content=pdf_bytes,
...
class MaterialConsumptionLine(BaseModel):
    material_sku_name: str
    item_type: str = 'rm'
    actual_consumed_qty: float | None = None
    return_qty: float = 0
    uom: str | None = None
...
class ByproductRequest(BaseModel):
    lines: list[ByproductLine]

class MaterialAccountingRequest(BaseModel):
    total_material_issued_kg: float | None = None
    fg_output_kg: float = 0
    process_loss_kg: float = 0
    process_loss_sub_categories: dict | None = None
    extra_give_away_kg: float = 0
    balance_material_kg: float = 0
    offgrade_total_kg: float = 0
    rejection_kg: float = 0
    wastage_kg: float = 0
    control_sample_kg: float = 0
...
async def assign_jc(request: Request, job_card_id: int, body: AssignRequest,
                     user=Depends(require_permission("production", "job_cards", "lifecycle", "edit"))):
...
async def record_jc_output(request: Request, job_card_id: int, body: RecordOutputRequest,
                            user=Depends(require_permission("production", "job_cards", "output", "create"))):
...
async def sign_off_jc(request: Request, job_card_id: int, body: SignOffRequest,
                       user=Depends(require_permission("production", "job_cards", "sign_offs", "create"))):
...
async def close_jc(request: Request, job_card_id: int):
    ...
    if "error" in result:
        if result["error"] == "incomplete_accounting":
            raise HTTPException(status_code=400, detail={"error": "incomplete_accounting", "missing_sections": result["missing_sections"]})
        raise HTTPException(status_code=400, detail=result.get("message", result["error"]))
    return result


@router.put("/job-cards/{job_card_id}/material-consumption")
async def save_jc_material_consumption(request: Request, job_card_id: int, body: MaterialConsumptionRequest):
    """Save per-BOM-line actual material consumption."""
    from app.modules.production.services.job_card_engine import save_material_consumption
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
        async with conn.transaction():
            result = await save_material_consumption(conn, job_card_id, [l.model_dump() for l in body.lines])
    if "error" in result:
        raise HTTPException(status_code=400, detail=result.get("message", result["error"]))
    return result


@router.put("/job-cards/{job_card_id}/byproducts")
async def save_jc_byproducts(request: Request, job_card_id: int, body: ByproductRequest):
    """Save by-product / off-grade category entries."""
    from app.modules.production.services.job_card_engine import save_byproducts
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
        async with conn.transaction():
            result = await save_byproducts(conn, job_card_id, [l.model_dump() for l in body.lines])
    if "error" in result:
        raise HTTPException(status_code=400, detail=result.get("message", result["error"]))
    return result


@router.put("/job-cards/{job_card_id}/material-accounting")
async def save_jc_material_accounting(request: Request, job_card_id: int, body: MaterialAccountingRequest):
    """Save and validate material accounting summary."""
    from app.modules.production.services.job_card_engine import save_material_accounting
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
        async with conn.transaction():
            result = await save_material_accounting(conn, job_card_id, body.model_dump())
    if "error" in result:
        raise HTTPException(status_code=400, detail=result.get("message", result["error"]))
    return result
...
class FifoSkipBody(BaseModel):
    ...
    @validator("job_card_id", "batch_id", "skipped_by", "block_for_so", pre=True)
