...
async def generate_draft_indents(conn, mrp_result: dict, plan_id: int, entity: str) -> dict:
    ...
    if indents:
        await events.indent_drafted(entity, plan_id=plan_id, count=len(indents), total_shortage_kg=sum(i["required_qty_kg"] for i in indents))
...
async def send_indent(conn, indent_id: int) -> dict:
    ...
    await events.indent_sent(indent['entity'], indent_id=indent_id, material=material, qty_kg=qty, deadline=str(deadline))
