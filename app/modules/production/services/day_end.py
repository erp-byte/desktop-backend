...
async def get_day_end_summary(conn, entity: str, target_date: date | None = None) -> dict:
    ...
    rows = await conn.fetch(
        ...
        SELECT jc.job_card_id, jc.job_card_number, jc.fg_sku_name, jc.customer_name,
               ...
               o.process_loss_kg, o.offgrade_kg, o.dispatch_qty
        FROM job_card jc
...
async def bulk_dispatch(conn, dispatches: list[dict], entity: str) -> dict:
    """Bulk update dispatch_qty for multiple job cards."""
    updated = 0
    for d in dispatches:
        jc_id = d['job_card_id']
        qty = d['dispatch_qty']
        ...
        await conn.execute(
            "UPDATE job_card_output SET dispatch_qty = $2 WHERE job_card_id = $1",
            jc_id, qty,
        )
