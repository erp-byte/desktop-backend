...
# ---------------------------------------------------------------------------
# Startup skip-guard — run the full ingest ONCE per version, then stay quiet
# ---------------------------------------------------------------------------
# The cascade below is idempotent but NOT free: every boot it re-opens several
# Excel/CSV plugs and re-queries/upserts thousands of rows only to write nothing
# new (the "0 created / already-present" logs). That made every uvicorn restart
# slow and noisy. We now stamp a completion marker keyed on INGEST_VERSION: once
# a full pass succeeds, later boots at the SAME version skip the whole cascade.
#
# Bump INGEST_VERSION whenever a plug file or any ingest step changes so the next
# boot re-runs the backfill exactly once; or set MASTER_INGEST_FORCE=1 for a
# one-off forced re-run without a code change. (Per-step "is it done?" guards
# don't work here — jc_routing/jc_bom carry permanently-unresolved rows, so they
# would never register as complete.)
INGEST_VERSION = "1"
_INGEST_META_KEY = "master_ingest_version"


async def _ensure_ingest_meta(pool) -> bool:
    """Create the tiny ingest_meta marker table if absent. Returns False (which
    disables the skip-guard) when the table can't be provisioned — e.g. a DB role
    without CREATE — so startup degrades to the previous always-run behaviour
    instead of crashing."""
    try:
        if await pool.fetchval("SELECT to_regclass('ingest_meta')") is None:
            await pool.execute(
                """
        ...
        return True
    except Exception as e:  # defensive: permissions / concurrent-create race
        logger.warning("ingest_meta unavailable (%s); running full ingest", e)
        return False


async def _ingest_up_to_date(pool) -> bool:
    stored = await pool.fetchval(
        "SELECT value FROM ingest_meta WHERE key = $1", _INGEST_META_KEY
    )
    return stored == INGEST_VERSION


async def _mark_ingest_done(pool) -> None:
    await pool.execute(
        """
        ...
        """,
        _INGEST_META_KEY, INGEST_VERSION,
    )
...
async def run_master_ingest(pool, data_dir: Path, master_items: list) -> dict | None:
    ...
    Skips the whole cascade on a boot whose ingest_meta marker already matches
    INGEST_VERSION (unless MASTER_INGEST_FORCE=1), so a warm restart no longer
    re-reads the plugs just to write nothing."""

    force = os.environ.get("MASTER_INGEST_FORCE", "").strip().lower() in {
        "1", "true", "yes", "on",
    }
    guarded = await _ensure_ingest_meta(pool)
    if guarded and not force and await _ingest_up_to_date(pool):
        logger.info(
            "Master ingest already complete (v%s) — skipping startup ingest "
            "(set MASTER_INGEST_FORCE=1 or bump INGEST_VERSION to re-run)",
            INGEST_VERSION,
        )
        return None
