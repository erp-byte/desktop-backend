...
def generate_job_card_pdf(jc_data: dict, mode: str = 'full') -> bytes:
    ...
    if mode == 'full':
        ...
        # NOTE: `material_return_kg` / `rejection_kg` columns on job_card_output were dropped
        # by migration 011. Display the replacement fields: net_output_kg (fg+byproducts)
        # and the aggregated byproduct total (available via acct.offgrade_total_kg).
        info_row('RM Consumed (Kg)', _fmt_num(output.get('rm_consumed_kg')), 'Net Output (Kg)', _fmt_num(output.get('net_output_kg')))
        info_row('Yield %', _fmt_num(output.get('yield_pct')), 'Off-grade (Kg)', _fmt_num(acct.get('offgrade_total_kg')))
