...
async def get_fulfillment_list(conn, *, entity=None, status=None, financial_year=None,
    ...
    # Fetch page
    # so_line is LEFT-joined (not INNER) because so_line_id can be NULL on
    # carryforward / manually-created fulfillment records — losing those rows
    # in the listing would silently break the plan-builder. l.uom is a TEXT
    # column holding the units-per-kg multiplier (e.g. "10" for a 100g pack);
    # the frontend coerces and uses it to display Production Qty in units.
    offset = (page - 1) * page_size
    rows = await conn.fetch(
        f"""
        SELECT f.*, h.so_number AS so_number, h.so_date, l.uom AS uom
        FROM so_fulfillment f
        LEFT JOIN so_header h ON f.so_id = h.so_id
        LEFT JOIN so_line l ON f.so_line_id = l.so_line_id
        WHERE {where}
