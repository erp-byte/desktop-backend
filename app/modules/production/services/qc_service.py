...
async def submit_inspection(conn, inspection_id, *, result, findings=None,
    ...
    # Derive entity from the associated job card
    qi_row = await conn.fetchrow(
        "SELECT qi.job_card_id, jc.entity FROM qc_inspection qi JOIN job_card jc ON qi.job_card_id = jc.job_card_id WHERE qi.inspection_id = $1 OR qi.id::text = $1",
        str(inspection_id),
    )
    qc_entity = qi_row["entity"] if qi_row else "cfpl"

    if result in ("pass", "conditional_pass"):
        await events.qc_passed(qc_entity, inspection_id=str(inspection_id), findings=findings)
    else:
        await events.qc_failed(qc_entity, inspection_id=str(inspection_id), findings=findings)
