...
async def receive_material_via_qr(conn, job_card_id: int, box_ids: list[str], entity: str) -> dict:
    ...
    # Gate: check store allocation status
    store_status = jc.get('store_allocation_status', 'pending')
    if store_status == 'rejected':
        return {"error": "store_rejected", "message": "Store has rejected material allocation for this job card"}
    if store_status == 'pending':
        return {"error": "store_pending", "message": "Awaiting store approval before material can be received"}
    ...
    for box_id in box_ids:
        ...
        for indent in rm_indents:
            # Skip if store has not approved this indent line
            indent_decision = indent.get('store_decision', 'pending')
            if indent_decision in ('rejected', 'pending'):
                continue
...
async def manual_acknowledge_material(conn, job_card_id: int, indent_lines: list[dict],
    ...
    # Same store gate as QR
    store_status = jc.get('store_allocation_status', 'pending')
    if store_status == 'rejected':
        return {"error": "store_rejected", "message": "Store has rejected material allocation for this job card"}
    if store_status == 'pending':
        return {"error": "store_pending", "message": "Awaiting store approval before material can be acknowledged"}
    ...
    for line in indent_lines:
        ...
        # Must be store-approved and not already fulfilled
        sd = row.get('store_decision', 'approved')
        if sd not in ('approved', 'partial'):
            skipped.append({"indent_type": indent_type, "indent_id": indent_id, "reason": f"store_{sd}"})
            continue
        ...
        if sd == 'partial' and row.get('store_approved_qty'):
            issued = float(row['store_approved_qty'])
        else:
...
async def manual_acknowledge_all(conn, job_card_id: int, acknowledged_by: str, entity: str) -> dict:
    """Acknowledge ALL eligible (store-approved, unfulfilled) indent lines for a job card."""
    indent_lines = []

    rm_rows = await conn.fetch(
        "SELECT rm_indent_id FROM job_card_rm_indent WHERE job_card_id = $1 AND status != 'fulfilled' AND COALESCE(store_decision, 'approved') IN ('approved', 'partial')",
        job_card_id,
    )
    for r in rm_rows:
    ...
    pm_rows = await conn.fetch(
        "SELECT pm_indent_id FROM job_card_pm_indent WHERE job_card_id = $1 AND status != 'fulfilled' AND COALESCE(store_decision, 'approved') IN ('approved', 'partial')",
        job_card_id,
    )
    for r in pm_rows:
