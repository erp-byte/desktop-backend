...
9. FLOOR/MACHINE CONSTRAINTS: When user_constraints are provided for a fulfillment item:
   ...
   c. When floor_qty_kg is provided, use those exact quantities per floor. The user has already split the total production qty across floors. Respect their split.
   d. When floor_qty_kg is NOT provided, split quantity equally across floors by default, then adjust proportionally based on machine capacity.
   e. Each schedule line must include a "floor" field matching the floor name.
   f. If no user_constraints are provided for an item, assign machines freely as before.
...
async def collect_planning_context(conn, entity: str, target_date: date,
                                    ...
                                    user_constraints: list[dict] | None = None) -> dict:
    ...
    logger.info("Planning context: %d demand items, %d no-BOM, %d machines, %d constraints",
                len(demand), len(no_bom), len(all_machines), len(user_constraints or []))
    return context
