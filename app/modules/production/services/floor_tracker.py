...
async def move_material(conn, sku_name: str, from_location: str, to_location: str,
    ...
    # Update inventory_batch floor locations for matching batches
    if scanned_qr_codes:
        # If specific boxes/batches scanned, update those
        for qr in scanned_qr_codes:
            await conn.execute("""
                UPDATE inventory_batch SET floor_id = $2, updated_at = NOW()
                WHERE batch_id = $1
            """, qr, to_location)
    else:
        # Update FIFO batches for this SKU at the source location
        await conn.execute("""
            UPDATE inventory_batch SET floor_id = $3, updated_at = NOW()
            WHERE batch_id IN (
                SELECT batch_id FROM inventory_batch
                WHERE sku_name ILIKE $1 AND floor_id = $2 AND entity = $4
                  AND status IN ('AVAILABLE', 'BLOCKED') AND current_qty_kg > 0
                ORDER BY inward_date ASC LIMIT 1
            )
        """, f"%{sku_name}%", from_location, to_location, entity)
    ...
    try:
        ...
        logger.exception("material_moved emit failed; swallowing")
