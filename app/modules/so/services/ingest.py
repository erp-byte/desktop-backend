    ...
    async with pool.acquire() as conn:
        async with conn.transaction():
            for so in parsed_orders:
                ...
                # Bug 9: Raise a clear duplicate error rather than letting the DB
                # raise a cryptic unique violation mid-transaction.
                exists = await conn.fetchval(
                    "SELECT 1 FROM so_header WHERE so_number = $1", so["so_number"]
                )
                if exists:
                    raise ValueError(
                        f"SO '{so['so_number']}' already exists. "
                        "Remove duplicates from the file and re-upload."
                    )
