...
async def run_integrity_check(conn, *, emit: bool = True) -> list[dict]:
    ...
    rows = await conn.fetch(
        """
        SELECT sr.id, sr.request_id, sr.warehouse,
    ...
    rows = await conn.fetch(
        """
        SELECT sr.id, sr.request_id, sr.warehouse, sr.linked_job_card_id, jc.jobcard_type
          FROM sample_requisitions sr
    ...
    if emit:
        for d in drifts:
            await notification_service.emit_alert(
                ...
                message=f"Sample integrity drift on {d['request_id']}: {d['check']} — {d['detail']}",
                related_id=d["requisition_id"])
    return drifts


def _d(row, check: str, detail: str) -> dict:
    return {"requisition_id": row["id"], "request_id": row["request_id"],
