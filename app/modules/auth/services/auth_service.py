...
async def login(conn, phone: str, password: str, ip_address: str = None, user_agent: str = None) -> dict:
    ...
    logger.warning("LOGIN_DEBUG phone=%r len=%d pw_len=%d ip=%s",
                   phone, len(phone or ""), len(password or ""), ip_address)

    user = await conn.fetchrow(
        "SELECT * FROM auth_user WHERE phone = $1 AND is_active = TRUE", phone,
    )
    if not user:
        logger.warning("LOGIN_DEBUG no user row for phone=%r", phone)
    ...
    if not verify_password(password, user['password_encrypted']):
        logger.warning("LOGIN_DEBUG password mismatch for phone=%r user_id=%s", phone, user['user_id'])
        return {"error": "invalid_credentials", "message": "Invalid phone number or password"}
    ...
    # HIGH-4: log user_id only; name / phone are PII under DPDP 2023.
...
async def create_user(conn, phone: str, password: str, full_name: str,
                       role_id: int, email: str = None, entity: str = None,
                       allowed_warehouses: list = None) -> dict:
    """Create a new user with encrypted password."""
    encrypted = encrypt_password(password)

    user_id = await conn.fetchval(
        """
        INSERT INTO auth_user (phone, password_encrypted, full_name, email, role_id, entity, allowed_warehouses)
        VALUES ($1, $2, $3, $4, $5, $6, $7)
        RETURNING user_id
        """,
        phone, encrypted, full_name, email, role_id, entity, allowed_warehouses,
    )
