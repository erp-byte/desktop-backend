...
class Settings(BaseSettings):
    ...
    # ── Packing module (encrypted batch tokens) ───────────────────────────
    # AES-256-GCM key (urlsafe-base64 of 32 bytes) for encrypting/decrypting
    # the packing "batch token". Server-only; never sent to the client.
