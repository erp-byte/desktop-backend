...
# SQL fragment that applies the same canonicalisation as
# ``normalise_warehouse_code`` to a column or text expression. Plug it in
# wherever a plain ``factory = $1`` was used:
#
#     conditions.append(f"{WAREHOUSE_NORM_SQL('factory')} = {WAREHOUSE_NORM_SQL(f'${idx}')}")
#
# For ANY-of-array matching the param side must also be normalised — use
# the inline subquery in WAREHOUSE_NORM_ANY_SQL below.
...
def WAREHOUSE_NORM_ANY_SQL(col_expr: str, param_placeholder: str) -> str:
    """ANY-of-text[] match with both sides normalised.

    Example::

        sql = WAREHOUSE_NORM_ANY_SQL("factory", "$3")
        # → factory's canonical form ∈ (canonical form of each elem in $3::text[])

    Use when the caller passes a list of allowed warehouse codes and the
    column may hold a differently-formatted variant of the same site."""
    return (
