...
-- ═══════════════════════════════════════════════════════════════════
-- Migration: Backfill NULL customer_name on plan lines from ai_analysis_json
-- ═══════════════════════════════════════════════════════════════════

-- Step 1: Backfill from linked fulfillment IDs (if present)
UPDATE production_plan_line pl
SET customer_name = f.customer_name
FROM so_fulfillment f
WHERE pl.customer_name IS NULL
  AND pl.linked_so_fulfillment_ids IS NOT NULL
  AND array_length(pl.linked_so_fulfillment_ids, 1) > 0
  AND f.fulfillment_id = pl.linked_so_fulfillment_ids[1];

-- Step 2: Backfill from ai_analysis_json schedule items (match by fg_sku_name + plan)
-- Handles cases where Claude used "customer" instead of "customer_name"
-- and "fulfillment_id" instead of "linked_fulfillment_ids"
UPDATE production_plan_line pl
SET customer_name = COALESCE(
    (sched->>'customer_name'),
    (sched->>'customer')
),
    linked_so_fulfillment_ids = CASE
        WHEN pl.linked_so_fulfillment_ids IS NULL AND (sched->>'fulfillment_id') IS NOT NULL
        THEN ARRAY[(sched->>'fulfillment_id')::INT]
        ELSE pl.linked_so_fulfillment_ids
    END,
    reasoning = COALESCE(pl.reasoning, sched->>'reasoning', sched->>'notes')
FROM production_plan p,
     LATERAL jsonb_array_elements(
         (p.ai_analysis_json::jsonb)->'schedule'
     ) WITH ORDINALITY AS t(sched, idx)
WHERE pl.plan_id = p.plan_id
  AND pl.customer_name IS NULL
  AND p.ai_analysis_json IS NOT NULL
  AND LOWER(TRIM(sched->>'fg_sku_name')) = LOWER(TRIM(pl.fg_sku_name));

-- Step 3: For any still-null customer_name, try the fulfillment fallback again
UPDATE production_plan_line pl
SET customer_name = f.customer_name
FROM so_fulfillment f
WHERE pl.customer_name IS NULL
...
-- Propagate customer_name to production_order from plan lines
UPDATE production_order po
SET customer_name = pl.customer_name
FROM production_plan_line pl
WHERE po.customer_name IS NULL
  AND po.plan_line_id = pl.plan_line_id
  AND pl.customer_name IS NOT NULL;

-- ═══════════════════════════════════════════════════════════════════
-- Migration: Job Card Output v2 — Balance Material, Byproduct, Loss Recon
-- 2026-04-10
-- ═══════════════════════════════════════════════════════════════════

-- 1. Create job_card_balance_material table
CREATE TABLE IF NOT EXISTS job_card_balance_material (
    balance_id      SERIAL PRIMARY KEY,
    job_card_id     INT NOT NULL REFERENCES job_card(job_card_id),
    material_id     INT,
    material_name   TEXT NOT NULL,
    balance_type    TEXT NOT NULL CHECK (balance_type IN ('extra_given', 'returned', 'wastage', 'control_sample')),
    qty_kg          NUMERIC(15,3) NOT NULL DEFAULT 0,
    remarks         TEXT,
    created_at      TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
CREATE INDEX IF NOT EXISTS idx_balance_mat_jc ON job_card_balance_material(job_card_id);

-- 2. Add uom to job_card_byproduct
ALTER TABLE job_card_byproduct ADD COLUMN IF NOT EXISTS uom TEXT DEFAULT 'kg';

-- 3. Add net_output_kg and yield_pct to job_card_output
ALTER TABLE job_card_output ADD COLUMN IF NOT EXISTS net_output_kg NUMERIC(15,3) DEFAULT 0;
ALTER TABLE job_card_output ADD COLUMN IF NOT EXISTS yield_pct NUMERIC(8,3) DEFAULT 0;

-- 4. Add UNIQUE constraint to job_card_loss_reconciliation for upsert support (idempotent)
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint WHERE conname = 'uq_loss_recon_jc_category'
    ) THEN
...
-- NOTE: Steps 5–9 (data migration from dropped columns) were run once manually on 2026-04-10
--       and are omitted here to keep startup idempotent. Those columns no longer exist.

-- 10. Backfill net_output_kg and yield_pct on existing rows
WITH byproduct_totals AS (
    SELECT job_card_id, SUM(quantity_kg) AS total_bp_kg
    FROM job_card_byproduct
    GROUP BY job_card_id
)
UPDATE job_card_output o
SET
    net_output_kg = COALESCE(o.fg_actual_kg, 0) + COALESCE(bt.total_bp_kg, 0),
    yield_pct = CASE
        WHEN o.rm_consumed_kg IS NOT NULL AND o.rm_consumed_kg > 0
        THEN ROUND(
            (COALESCE(o.fg_actual_kg, 0) + COALESCE(bt.total_bp_kg, 0))
            / o.rm_consumed_kg * 100, 3)
        ELSE 0
    END
FROM byproduct_totals bt
WHERE bt.job_card_id = o.job_card_id;

-- Rows with no byproducts
UPDATE job_card_output
SET
    net_output_kg = COALESCE(fg_actual_kg, 0),
    yield_pct = CASE
        WHEN rm_consumed_kg IS NOT NULL AND rm_consumed_kg > 0
        THEN ROUND(COALESCE(fg_actual_kg, 0) / rm_consumed_kg * 100, 3)
        ELSE 0
    END
WHERE (net_output_kg = 0 OR net_output_kg IS NULL)
  AND fg_actual_kg IS NOT NULL;

-- 11. Drop deprecated columns from job_card_output
ALTER TABLE job_card_output DROP COLUMN IF EXISTS material_return_kg;
ALTER TABLE job_card_output DROP COLUMN IF EXISTS rejection_kg;
ALTER TABLE job_card_output DROP COLUMN IF EXISTS rejection_reason;
ALTER TABLE job_card_output DROP COLUMN IF EXISTS process_loss_pct;
ALTER TABLE job_card_output DROP COLUMN IF EXISTS offgrade_kg;
ALTER TABLE job_card_output DROP COLUMN IF EXISTS offgrade_category;
ALTER TABLE job_card_output DROP COLUMN IF EXISTS dispatch_qty;
