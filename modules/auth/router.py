...
from modules.auth.middleware import AuthUser, get_current_user
...
from modules.auth.services import auth_service, password_rules, rate_limiter
from modules.auth.services.phone import normalize as normalize_phone
...
async def _require_auth(request: Request) -> dict:
    ...
    from modules.auth.services.auth_service import validate_session
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
...
async def create_user(request: Request, body: CreateUserRequest):
    ...
    from modules.auth.services.auth_service import create_user as _create
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
        try:
