...
from modules.auth.middleware import AuthUser, require_permission
from modules.auth.services.permission_service import check_permission
from modules.production.schemas.job_card_edit import (
...
async def sync_fulfillment(request: Request, body: FulfillmentSyncRequest):
    ...
    from modules.production.services.fulfillment import sync_fulfillment as _sync
    pool = request.app.state.db_pool
...
async def list_fulfillment(
    ...
    from modules.production.services.fulfillment import get_fulfillment_list
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
...
async def demand_summary(
    ...
    from modules.production.services.fulfillment import get_demand_summary
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
...
async def chart_summary(
    ...
    from modules.production.services.fulfillment import get_chart_summary
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
...
async def filter_options(
    ...
    """
    from modules.production.services.fulfillment import get_filter_options
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
...
async def customer_view(
    ...
    from modules.production.services.fulfillment import get_enriched_fulfillment
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
...
async def fy_review(
    ...
    from modules.production.services.fulfillment import get_fy_review
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
...
async def get_fulfillment_detail_endpoint(request: Request, fulfillment_id: int):
    ...
    from modules.production.services.fulfillment import get_fulfillment_detail
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
...
async def get_bom_override(request: Request, fulfillment_id: int):
    ...
    from modules.production.services.fulfillment import get_bom_overrides
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
...
async def save_bom_override(request: Request, fulfillment_id: int, body: BomOverrideRequest):
    ...
    from modules.production.services.fulfillment import save_bom_overrides
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
        async with conn.transaction():
...
async def get_floor_stock(request: Request, fulfillment_id: int):
    ...
    from modules.production.services.fulfillment import get_floor_stock
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
...
async def save_floor_stock(request: Request, fulfillment_id: int, body: FloorStockRequest):
    ...
    from modules.production.services.fulfillment import save_floor_stock
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
        async with conn.transaction():
...
async def list_floors(request: Request, entity: str = Query(None)):
    ...
    from modules.production.services.fulfillment import get_floor_locations
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
...
async def carryforward(request: Request, body: CarryforwardRequest):
    ...
    from modules.production.services.fulfillment import carryforward_orders
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
        async with conn.transaction():
...
async def revise(request: Request, fulfillment_id: int, body: ReviseRequest):
    ...
    from modules.production.services.fulfillment import revise_order
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
        async with deferred_events():
            async with conn.transaction():
...
async def create_plan_with_ai(request: Request, body: CreatePlanWithAIRequest):
    ...
    """
    from modules.production.services.ai_planner import (
...
async def mrp_availability(
    ...
    from modules.production.services.mrp import check_availability
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
...
async def create_orders_from_plan(request: Request, body: CreateOrdersRequest):
    ...
    from modules.production.services.job_card_engine import create_production_orders
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
...
async def store_pending_allocations(
    ...
    from modules.production.services.store_controller import get_pending_allocations
    if size is not None:
        page_size = size
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
...
async def store_dashboard(request: Request, entity: str = Query(...)):
    ...
    from modules.production.services.store_controller import get_store_dashboard
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
...
async def store_decide(request: Request, body: AllocationRequest, entity: str = Query(...)):
    ...
    from modules.production.services.store_controller import decide_allocation
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
        async with deferred_events():
            async with conn.transaction():
...
async def store_verify_floor(request: Request, body: FloorVerifyRequest, entity: str = Query(...)):
    ...
    from modules.production.services.store_controller import verify_floor_stock
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
        async with conn.transaction():
...
async def store_suggest_alt(request: Request, body: SuggestAlternativeRequest, entity: str = Query(...)):
    ...
    from modules.production.services.store_controller import suggest_alternative
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
        async with conn.transaction():
...
async def job_card_allocations(request: Request, job_card_id: int):
    ...
    from modules.production.services.store_controller import get_allocation_summary
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
...
async def dispatch_to_next(
    ...
    from modules.production.services.job_card_engine import dispatch_partial_to_next_stage
...
async def list_batches(
    ...
    from modules.production.services.inventory_service import get_available_batches, get_inventory_summary
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
        if sku_name:
...
async def get_batch_detail(request: Request, batch_id: str):
    from modules.production.services.inventory_service import get_batch, get_batch_history
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
...
async def flag_batch_endpoint(request: Request, batch_id: str, body: BatchFlagRequest):
    from modules.production.services.inventory_service import flag_batch
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
        async with conn.transaction():
...
async def block_batch_endpoint(request: Request, batch_id: str, body: BatchBlockRequest):
    from modules.production.services.inventory_service import block_batch
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
        async with conn.transaction():
...
async def force_reassign_endpoint(request: Request, batch_id: str, body: ForceReassignRequest,
                                   entity: str = Query(...)):
    from modules.production.services.inventory_service import force_reassign_batch
...
async def legacy_import_endpoint(request: Request, body: LegacyImportRequest,
                                  entity: str = Query(...)):
    from modules.production.services.inventory_service import import_legacy_batches
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
        async with conn.transaction():
...
async def create_internal_issue_endpoint(request: Request, body: InternalIssueRequest,
                                          entity: str = Query(...)):
    from modules.production.services.inventory_service import create_internal_issue
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
        async with conn.transaction():
...
async def approve_internal_issue_endpoint(request: Request, note_id: int,
    ...
    from modules.production.services.inventory_service import approve_internal_issue
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
        async with conn.transaction():
...
async def check_shortfall_endpoint(request: Request,
                                    ...
                                    job_card_id: int = Query(None)):
    from modules.production.services.inventory_service import check_shortfall
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
...
async def reconcile_endpoint(request: Request, entity: str = Query(...)):
    from modules.production.services.inventory_service import reconcile_quantities
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
...
async def reject_batch_endpoint(request: Request, batch_id: str, body: BatchRejectRequest,
                                 entity: str = Query(...)):
    from modules.production.services.inventory_service import log_batch_rejection
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
        async with conn.transaction():
...
async def resolve_batch_endpoint(request: Request, batch_id: str, body: ResolveFlagRequest):
    from modules.production.services.inventory_service import resolve_flagged_batch
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
        async with conn.transaction():
...
async def return_batch_endpoint(request: Request, batch_id: str, body: ReturnBatchRequest):
    from modules.production.services.inventory_service import return_batch
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
        async with conn.transaction():
...
async def batch_rejections_endpoint(request: Request, batch_id: str):
    from modules.production.services.inventory_service import get_batch_rejections
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
...
async def approve_constrained_endpoint(request: Request, note_id: int,
    ...
    from modules.production.services.inventory_service import approve_internal_issue_with_space_constraint
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
        async with conn.transaction():
...
async def reject_internal_issue_endpoint(request: Request, note_id: int, body: RejectIssueRequest):
    from modules.production.services.inventory_service import reject_internal_issue
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
        async with conn.transaction():
...
async def list_internal_issues_endpoint(request: Request, entity: str = Query(...),
                                         status: str = Query(None)):
    from modules.production.services.inventory_service import list_internal_issues
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
...
async def generate_job_cards(request: Request, body: GenerateJobCardsRequest):
    ...
    from modules.production.services.job_card_engine import create_job_cards
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
        async with deferred_events():
            async with conn.transaction():
...
async def get_job_card(request: Request, job_card_id: int):
    ...
    from modules.production.services.job_card_engine import get_job_card_detail
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
...
async def update_job_card(request: Request, job_card_id: int, body: JobCardPatchRequest):
    ...
    from modules.production.services import jc_editor
    from app.webhooks import events
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
        async with deferred_events():
...
async def assign_jc(request: Request, job_card_id: int, body: AssignRequest):
    from modules.production.services.job_card_engine import assign_job_card
    from app.webhooks import events
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
...
async def acknowledge_material(request: Request, job_card_id: int, body: ManualAckRequest):
    from modules.production.services.qr_service import manual_acknowledge_material, manual_acknowledge_all
    from app.webhooks import events
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
        jc = await conn.fetchrow("SELECT entity, job_card_number FROM job_card WHERE job_card_id = $1", job_card_id)
...
async def start_jc(request: Request, job_card_id: int):
    from modules.production.services.job_card_engine import start_job_card
    from app.webhooks import events
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
...
async def complete_step(request: Request, job_card_id: int, body: CompleteStepRequest):
    from modules.production.services.job_card_engine import complete_process_step
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
...
async def record_jc_output(request: Request, job_card_id: int, body: RecordOutputRequest):
    from modules.production.services.job_card_engine import record_output
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
        async with conn.transaction():
...
async def complete_jc(request: Request, job_card_id: int):
    from modules.production.services.job_card_engine import complete_job_card
    from app.webhooks import events
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
...
async def sign_off_jc(request: Request, job_card_id: int, body: SignOffRequest):
    from modules.production.services.job_card_engine import sign_off
    from app.webhooks import events
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
        jc = await conn.fetchrow("SELECT job_card_number, entity FROM job_card WHERE job_card_id = $1", job_card_id)
...
async def close_jc(request: Request, job_card_id: int):
    from modules.production.services.job_card_engine import close_job_card
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
...
async def force_unlock_jc(request: Request, job_card_id: int, body: ForceUnlockRequest):
    from modules.production.services.job_card_engine import force_unlock
    from app.webhooks import events
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
        jc = await conn.fetchrow("SELECT entity, job_card_number FROM job_card WHERE job_card_id = $1", job_card_id)
...
async def list_floor_inventory(
    ...
    from modules.production.services.floor_tracker import get_floor_detail
    pool = request.app.state.db_pool
...
async def floor_summary(request: Request, entity: str = Query(...)):
    ...
    from modules.production.services.floor_tracker import get_floor_summary
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
...
async def move_material_endpoint(request: Request, body: MoveRequest):
    ...
    from modules.production.services.floor_tracker import move_material
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
        async with deferred_events():
            async with conn.transaction():
...
async def movement_history(
    ...
    from modules.production.services.floor_tracker import get_movement_history
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
...
async def check_idle(request: Request, entity: str = Query(...)):
    ...
    from modules.production.services.idle_checker import check_idle_materials
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
        async with conn.transaction():
...
async def day_end_summary(
    ...
    from modules.production.services.day_end import get_day_end_summary
    pool = request.app.state.db_pool
...
async def submit_scan(request: Request, body: BalanceScanSubmitRequest):
    ...
    from modules.production.services.day_end import submit_balance_scan
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
        async with conn.transaction():
...
async def scan_status(
    ...
    from modules.production.services.day_end import get_scan_status
    pool = request.app.state.db_pool
    d = date.fromisoformat(target_date) if target_date else None
    async with pool.acquire() as conn:
...
async def scan_detail(request: Request, scan_id: int):
    ...
    from modules.production.services.day_end import get_scan_detail
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
...
async def reconcile_scan_endpoint(request: Request, scan_id: int, body: ReconcileRequest):
    ...
    from modules.production.services.day_end import reconcile_scan
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
        async with deferred_events():
            async with conn.transaction():
...
async def check_missing(request: Request, entity: str = Query(...), target_date: str = Query(None)):
    ...
    from modules.production.services.day_end import check_missing_scans
    pool = request.app.state.db_pool
    d = date.fromisoformat(target_date) if target_date else None
    async with pool.acquire() as conn:
        async with conn.transaction():
...
async def report_discrepancy_endpoint(request: Request, body: DiscrepancyReportRequest):
    ...
    from modules.production.services.discrepancy_manager import report_discrepancy
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
        async with deferred_events():
            async with conn.transaction():
...
async def get_discrepancy(request: Request, discrepancy_id: int):
    ...
    from modules.production.services.discrepancy_manager import get_discrepancy_detail
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
...
async def resolve_discrepancy_endpoint(request: Request, discrepancy_id: int, body: DiscrepancyResolveRequest):
    ...
    from modules.production.services.discrepancy_manager import resolve_discrepancy
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
...
async def list_production_indents(
    ...
    from modules.production.services.production_indent_service import list_production_indents as _list
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
...
async def get_production_indent(request: Request, indent_id: str):
    from modules.production.services.production_indent_service import get_production_indent as _get
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
...
async def create_production_indent(request: Request, body: ProductionIndentCreate):
    from modules.production.services.production_indent_service import create_production_indent as _create
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
        async with conn.transaction():
...
async def submit_production_indent(request: Request, indent_id: str):
    from modules.production.services.production_indent_service import submit_indent
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
...
async def approve_production_indent(request: Request, indent_id: str, body: CheckerAction):
    from modules.production.services.production_indent_service import approve_indent
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
...
async def return_production_indent(request: Request, indent_id: str, body: CheckerAction):
    from modules.production.services.production_indent_service import return_indent
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
...
async def cancel_production_indent(request: Request, indent_id: str, body: CancelBody):
    from modules.production.services.production_indent_service import cancel_indent
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
...
async def create_internal_order(request: Request, indent_id: str):
    from modules.production.services.production_indent_service import create_internal_order as _create
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
        async with conn.transaction():
...
async def get_lots(
    ...
    from modules.production.services.lot_issuance_service import get_lots as _get
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
...
async def get_lots_other_warehouses(
    ...
    from modules.production.services.lot_issuance_service import get_lots_other_warehouses as _get
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
        return await _get(conn, item_description=item_description,
...
async def fifo_skip(request: Request, body: FifoSkipBody):
    from modules.production.services.lot_issuance_service import record_fifo_skip
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
        async with conn.transaction():
...
async def force_assign(request: Request, body: ForceAssignBody):
    from modules.production.services.lot_issuance_service import force_assign_lot
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
        async with conn.transaction():
...
async def get_box(request: Request, box_id: str):
    from modules.production.services.lot_issuance_service import get_box as _get
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
...
async def create_issue_note(request: Request, body: IssueNoteCreate):
    from modules.production.services.lot_issuance_service import create_issue_note as _create
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
        async with conn.transaction():
...
async def raise_indent(request: Request, body: RaiseIndentBody):
    from modules.production.services.lot_issuance_service import raise_purchase_indent
    from app.webhooks import events
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
        async with deferred_events():
    ...
    if result.get("indent_id"):
        await events.indent_raised(body.entity or "cfpl", indent_id=result["indent_id"], material=body.material_sku_name, qty_kg=body.required_qty_kg, source="floor", job_card_id=body.job_card_id)
    return result
...
async def qc_queue(request: Request):
    from modules.production.services.qc_service import get_qc_queue
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
...
async def submit_qc_inspection(request: Request, inspection_id: str, body: QCInspectionBody):
    from modules.production.services.qc_service import submit_inspection
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
        async with deferred_events():
            async with conn.transaction():
...
async def list_rtv_dispositions(
    ...
    from modules.production.services.rtv_disposition_service import list_dispositions
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
...
async def assign_rtv_disposition(request: Request, body: RtvDispositionBody):
    from modules.production.services.rtv_disposition_service import assign_disposition
    from modules.production.services.mail_service import BUSINESS_HEADS
...
async def approve_discard(request: Request, body: DiscardBody):
    from modules.production.services.rtv_disposition_service import approve_discard
    from modules.production.services.mail_service import BUSINESS_HEADS
    if body.business_head and body.business_head not in BUSINESS_HEADS:
        raise HTTPException(
            status_code=400,
...
async def get_amendments(
    ...
    from modules.production.services.amendment_service import get_amendments as _get
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
...
async def get_amendment_count(
    ...
    from modules.production.services.amendment_service import get_amendment_count as _count
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
...
async def reconcile_batch_doc(request: Request, mat_doc_id: str):
    ...
    from modules.production.services.material_document_service import reconcile_batch
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
...
async def record_output_v2(request: Request, job_card_id: int, body: JobCardOutputV2Request):
    ...
    from modules.production.services.job_card_engine import record_output_v2 as _record
    from app.webhooks import events
    pool = request.app.state.db_pool
    async with pool.acquire() as conn:
        jc = await conn.fetchrow("SELECT job_card_number, entity FROM job_card WHERE job_card_id = $1", job_card_id)
