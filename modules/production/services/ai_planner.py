...
9. FLOOR/MACHINE CONSTRAINTS: When user_constraints are provided for a fulfillment item:
   a. You MUST only assign machines from the allowed_machines for that item. Never use machines outside the allowed list.
   b. Generate ONE schedule line per floor per item.
   c. When floor_qty_kg is provided, use those exact kg quantities per floor for qty_kg. The user has already split the total production qty across floors. Respect their split.
   d. When floor_qty_units is provided, use those exact unit counts per floor for qty_units. Do not recompute units from qty_kg / uom — the user has already split the total units across floors and that split is authoritative.
   e. When floor_qty_kg is NOT provided, split quantity equally across floors by default, then adjust proportionally based on machine capacity.
   f. Each schedule line must include a "floor" field matching the floor name.
   g. If no user_constraints are provided for an item, assign machines freely as before.
10. CLARIFYING QUESTIONS — TOKEN-EFFICIENT BATCHING:
   a. If ANYTHING is ambiguous, missing, or could be interpreted multiple ways, do NOT guess and do NOT fabricate a schedule.
   b. Instead, return an EMPTY "schedule" array and put EVERY clarifying question into the "questions" array — ALL of them, in a single response.
   c. Do not return a partial schedule alongside questions. Either you have enough information to commit to a full schedule (questions: []) or you don't (schedule: []).
   d. The user pays per Claude call. Asking questions one-at-a-time across multiple calls wastes their token budget. Batch every concern into this one response.
   e. Each question must be specific and answerable with a short text or a choice — not open-ended ("What should I do?" is bad; "Order #42 has no BOM — should I skip it, use BOM #15 from a similar SKU, or block the plan?" is good).
  ...
  "questions": [
    {
      ...
      "topic": "missing_bom",
      "fulfillment_id": 42,
      "question": "Order #42 (FG: Almonds 500g) has no active BOM. Skip this order, use a similar BOM, or block the plan?",
      "options": ["skip", "use_similar_bom:<bom_id>", "block"]
    }
  ]
}
"""
...
7. FLOOR/MACHINE CONSTRAINTS: When user_constraints are provided for a fulfillment item:
   a. You MUST only assign machines from the allowed_machines for that item. Never use machines outside the allowed list.
   b. Generate ONE schedule line per floor per item per day.
   c. When floor_qty_kg is provided, use those exact kg quantities per floor for qty_kg. The user has already split the total production qty across floors. Respect their split.
   d. When floor_qty_units is provided, use those exact unit counts per floor for qty_units. Do not recompute units from qty_kg / uom — the user has already split the total units across floors and that split is authoritative.
   e. When floor_qty_kg is NOT provided, split quantity equally across floors by default, then adjust proportionally based on machine capacity.
   f. Each schedule line must include a "floor" field matching the floor name.
   g. If no user_constraints are provided for an item, assign machines freely as before.
8. CLARIFYING QUESTIONS — TOKEN-EFFICIENT BATCHING:
   a. If ANYTHING is ambiguous, missing, or could be interpreted multiple ways, do NOT guess and do NOT fabricate a schedule.
   b. Instead, return an EMPTY "schedule" array and put EVERY clarifying question into the "questions" array — ALL of them, in a single response.
   c. Do not return a partial schedule alongside questions. Either you have enough information to commit to a full schedule (questions: []) or you don't (schedule: []).
   d. The user pays per Claude call. Asking questions one-at-a-time across multiple calls wastes their token budget. Batch every concern into this one response.
   e. Each question must be specific and answerable with a short text or a choice — not open-ended.
  ...
  "questions": [
    {
      ...
      "topic": "missing_bom",
      "fulfillment_id": 42,
      "question": "Order #42 has no active BOM. Skip, use a similar BOM, or block the plan?",
      "options": ["skip", "use_similar_bom:<bom_id>", "block"]
    }
  ]
}
...
8. CLARIFYING QUESTIONS — TOKEN-EFFICIENT BATCHING:
   a. If the change_event is ambiguous, or if the impact on existing lines could go multiple ways, do NOT guess a revision.
   b. Return an EMPTY "revised_schedule" array and put EVERY clarifying question into the "questions" array — ALL of them, in a single response.
   c. Do not return a partial revision alongside questions. Either commit to a full revised schedule (questions: []) or none of it (revised_schedule: []).
   d. The user pays per Claude call. Asking questions one-at-a-time across multiple calls wastes their token budget. Batch every concern into this one response.
...
Return this JSON structure:
{
  ...
  "questions": [
    {
      "id": "q1",
      "topic": "ambiguous_change_event",
      "plan_line_id": 2,
      "question": "Change event mentions 'Roaster A breakdown' but does not say for how long. Is it a 2-hour fix or a full-day outage?",
      "options": ["short_outage_2h", "full_day_outage", "indefinite"]
    }
  ]
}
"""
...
async def collect_planning_context(conn, entity: str, target_date: date,
                                    ...
                                    user_constraints: list[dict] | None = None,
                                    answered_questions: list[dict] | None = None) -> dict:
    """Gather all data Claude needs for plan generation.

    answered_questions: optional list of {id, question, answer} entries — when the
    previous Claude call returned questions, the user answers them and the answers
    flow back via this list so Claude has full context on the second call.
    """
    ...
    # Filter machines if user_constraints specify allowed machines
    all_machines = list(machines_map.values())
    if user_constraints:
        # Build set of all allowed machine names (across all constraints)
        allowed_names = set()
        for c in user_constraints:
            for floor_machines in c.get("allowed_machines", {}).values():
                allowed_names.update(m.lower() for m in floor_machines)
        if allowed_names:
            all_machines = [m for m in all_machines if m["name"].lower() in allowed_names]
    ...
    logger.info(
        "Planning context: %d demand items, %d no-BOM, %d machines, %d constraints, %d answers",
        len(demand), len(no_bom), len(all_machines),
        len(user_constraints or []), len(answered_questions or []),
    )
    return context
...
async def create_plan_from_ai(conn, entity: str, plan_type: str,
    ...
    Questions-mode: if Claude returned a non-empty `questions` array AND an empty
    `schedule`, do NOT write a plan — return questions to caller so the user can
    answer them and the backend can re-call Claude with the answers in context.
    """
    ...
    # Questions-mode: Claude needs clarification before generating a schedule.
    # Skip DB writes and surface questions to the caller.
    if questions and not schedule:
        logger.info("Claude returned %d clarifying questions; skipping plan creation", len(questions))
        return {
            "plan_id": None,
    ...
    for item in schedule:
        ...
        await conn.execute(
            ...
            INSERT INTO production_plan_line (
                ...
                linked_so_fulfillment_ids, reasoning, floor
            ...
            linked_ids or None, reasoning, floor_val,
        )
        lines_created += 1
...
async def collect_revision_context(conn, plan_id: int, change_event: str, entity: str,
                                    answered_questions: list[dict] | None = None) -> dict:
    """Collect current plan state + change event for revision.

    answered_questions: forwarded into the context when the previous Claude call
    on this revision asked questions and the user has now answered them.
    """
    ...
    return {
        ...
        **({"answered_questions": answered_questions} if answered_questions else {}),
    }
...
async def create_revised_plan(conn, old_plan_id: int, entity: str,
    ...
    Questions-mode: if Claude returned a non-empty `questions` array AND an empty
    `revised_schedule`, do NOT mark the old plan revised or create a new one —
    return questions to caller so the user can answer and the backend re-call
    Claude with the answers.
    """
    ...
    # Questions-mode: bail before any DB mutation. Old plan keeps its status.
    if questions and not revised_schedule:
        logger.info("Claude returned %d clarifying questions on revision; skipping write", len(questions))
        return {
            "old_plan_id": old_plan_id,
            "new_plan_id": None,
            "status": "needs_clarification",
            "revision_number": (old_plan['revision_number'] or 1),
            "lines_kept": 0,
            "lines_added": 0,
            "lines_cancelled": 0,
            "questions": questions,
    ...
    for item in revised_schedule:
        ...
        if action == "keep":
                ...
                if old_line:
                    await conn.execute(
                        ...
                        INSERT INTO production_plan_line (
                            ...
                            planned_qty_kg, planned_qty_units, machine_id,
                            priority, shift, stage_sequence, estimated_hours,
                            linked_so_fulfillment_ids, reasoning, status, floor
            ...
            # Coerce linked_fulfillment_ids from any-string-or-int → int[] for asyncpg
            raw_linked = item.get("linked_fulfillment_ids") or []
            linked_ids = []
            for x in raw_linked:
                try:
                    linked_ids.append(int(x))
                except (TypeError, ValueError):
                    logger.warning(
                        "create_revised_plan: dropping non-int linked_fulfillment_id %r "
                        "for fg_sku_name=%r",
                        x, fg_name,
                    )
            await conn.execute(
