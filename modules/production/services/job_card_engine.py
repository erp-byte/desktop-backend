...
async def create_production_orders(conn, plan_id: int, entity: str) -> dict:
    ...
    for pl in plan_lines:
        ...
        # Inherit floor from plan line first, fallback to BOM
        floor_val = pl.get('floor') or (bom['floors'][0] if bom.get('floors') else None)
        # Inherit machine_id from plan line
        machine_id_val = pl.get('machine_id')
        ...
        prod_order_id = await conn.fetchval(
            ...
            INSERT INTO production_order (
                ...
                total_stages, entity, factory, floor, machine_id, status
            ...
            total_stages, entity, factory, floor_val, machine_id_val,
        )
...
async def create_job_cards(conn, prod_order_id: int) -> dict:
    ...
    for step in route_steps:
        ...
        step_machine_id = order.get('machine_id')
        ...
        job_card_id = await conn.fetchval(
            ...
            step_machine_id,  # from plan line via production order, or None
        ...
        if rm_count > 0 or pm_count > 0:
            from modules.production.services.store_controller import create_pending_allocations
            await create_pending_allocations(conn, job_card_id, order['entity'])

            # Auto-raise purchase indents for any material shortfalls
            from modules.production.services.inventory_service import auto_raise_shortfall_indents
    ...
    await events.job_card_created(order['entity'], prod_order_id=prod_order_id, job_card_count=len(job_cards))
...
async def check_and_raise_floor_indents(conn, job_card_id: int, entity: str) -> dict:
    ...
    for ind in rm_indents:
        ...
        if shortfall > 0:
            ...
            # HI-04: use MAX(+1) inside the same transaction so same-request iterations
            # see prior inserts; retry once on unique-violation to handle the narrow
            # cross-transaction race. A DB sequence is a cleaner fix but requires a
            # migration — this is the minimal surgical change.
            for _attempt in range(3):
                seq = await conn.fetchval(
                    "SELECT COALESCE(MAX(CAST(SUBSTRING(indent_number FROM 14) AS INT)), 0) + 1 "
                    "FROM purchase_indent WHERE indent_number LIKE $1",
                    f"IND-{today_str}-%",
                )
                ...
                try:
                    await conn.execute(
                        ...
                        INSERT INTO purchase_indent (
                            indent_number, material_sku_name, required_qty_kg, entity,
                            job_card_id, indent_source, cascade_reason, status
                        ) VALUES ($1, $2, $3, $4, $5, 'floor_shortage', $6, 'raised')
                        """,
                        indent_number, ind['material_sku_name'], round(shortfall, 3), entity,
                        job_card_id,
    ...
    for ind in pm_indents:
        ...
        if shortfall > 0:
            today_str = date.today().strftime('%Y%m%d')
            # HI-04: see notes in the RM branch above.
            for _attempt in range(3):
                seq = await conn.fetchval(
                    "SELECT COALESCE(MAX(CAST(SUBSTRING(indent_number FROM 14) AS INT)), 0) + 1 "
                    "FROM purchase_indent WHERE indent_number LIKE $1",
...
async def assign_job_card(conn, job_card_id: int, team_leader: str, team_members: list[str] | None = None) -> dict:
    ...
    Material-receipt gating intentionally relaxed: callers may assign a JC
    regardless of its locked/unlocked status or store_allocation_status.
    Only blocks if the JC has progressed past assignment (in_progress, completed,
    closed, cancelled).
    """
...
async def start_job_card(conn, job_card_id: int) -> dict:
    ...
    Material-receipt gating intentionally relaxed: callers may start a JC
    regardless of whether materials have been received. Only blocks if the JC
    has already started or terminated (in_progress, completed, closed, cancelled).
    """
    jc = await conn.fetchrow("SELECT status FROM job_card WHERE job_card_id = $1", job_card_id)
    if not jc:
        return {"error": "not_found"}
    if jc['status'] in ('in_progress', 'completed', 'closed', 'cancelled'):
        return {"error": "invalid_status", "message": f"Cannot start job card in status '{jc['status']}'"}
...
async def record_output_v2(conn, job_card_id: int, data: dict) -> dict:
    ...
    jc = await conn.fetchrow(
        "SELECT batch_size_kg, fg_sku_name, stage, bom_id FROM job_card WHERE job_card_id = $1",
        job_card_id,
    )
    if not jc:
        return {"error": "not_found"}
    ...
    rm_consumed_lines = data.get('rm_consumed') or []
    if rm_consumed_lines:
        rm_consumed_kg = sum(float(r.get('consumed_qty_kg', 0)) for r in rm_consumed_lines)
    else:
        # Legacy v1 path / MCP tool: scalar total without BOM-line breakdown.
    ...
    if submitted_bom_line_ids and jc['bom_id']:
        ...
        if invalid_ids:
            return {
                ...
                "message": f"bom_line_id(s) {sorted(invalid_ids)} do not belong to this job card's BOM",
            }
    ...
    # 3. Replace balance materials (tied to BOM line)
    ...
    for bm in balance_materials:
        await conn.execute(
            ...
            INSERT INTO job_card_balance_material
                (job_card_id, bom_line_id, material_id, material_name, balance_type, qty_kg, remarks)
            VALUES ($1, $2, $3, $4, $5, $6, $7)
            """,
            job_card_id, bm.get('bom_line_id'), bm.get('material_id'), bm['material_name'],
    ...
    # ME-03: scope the update to a single post-production inspection (the most
    # recent pending one) so multi-checkpoint JCs don't have all pending
    # inspections collapsed into one result.
    if qc and qc.get('passed') is not None:
        await conn.execute(
            ...
            WHERE id = (
                ...
                ORDER BY
                    CASE WHEN checkpoint_type = 'post_production' THEN 0 ELSE 1 END,
                    created_at DESC
                LIMIT 1
            )
            """,
    ...
    return {
        "output_id": output_id,
        "fg_actual_kg": fg_actual_kg,
        "net_output_kg": net_output_kg,
        "yield_pct": yield_pct,
        "rm_consumed_kg": rm_consumed_kg,
        "rm_consumption_lines_saved": len(rm_consumed_lines),
...
async def close_job_card(conn, job_card_id: int) -> dict:
    ...
    Store sign-off (warehouse_incharge) gating intentionally relaxed: only
    production_incharge and quality_analysis sign-offs are required to close.
    """
    jc = await conn.fetchrow("SELECT status FROM job_card WHERE job_card_id = $1", job_card_id)
    if not jc:
        return {"error": "not_found"}
    ...
    # Check required sign-offs (store sign-off intentionally not required)
    ...
    required = {'production_incharge', 'quality_analysis'}
...
async def get_job_card_detail(conn, job_card_id: int) -> dict | None:
    ...
    metal = await conn.fetch(
        "SELECT * FROM job_card_metal_detection WHERE job_card_id = $1 AND deleted_at IS NULL ORDER BY detection_id", job_card_id,
    )
    weight = await conn.fetch(
        "SELECT * FROM job_card_weight_check WHERE job_card_id = $1 AND deleted_at IS NULL ORDER BY sample_number", job_card_id,
    )
    env = await conn.fetch(
        "SELECT * FROM job_card_environment WHERE job_card_id = $1 AND deleted_at IS NULL", job_card_id,
    )
    loss_recon = await conn.fetch(
        "SELECT * FROM job_card_loss_reconciliation WHERE job_card_id = $1 AND deleted_at IS NULL ORDER BY recon_id", job_card_id,
    )
    remarks = await conn.fetch(
        "SELECT * FROM job_card_remarks WHERE job_card_id = $1 AND deleted_at IS NULL ORDER BY remark_id", job_card_id,
    )
...
async def dispatch_partial_to_next_stage(
    ...
    if qty_kg <= 0:
        return {"error": f"qty_kg {qty_kg} invalid. Must be > 0"}

    # 1. Atomic conditional UPDATE on current JC — eliminates the read-validate-update
    # TOCTOU race. The WHERE clause enforces status, chain-has-next, and the cap
    # in a single statement; if zero rows match, we fall through to a precise error.
    new_dispatched_total = await conn.fetchval(
        """
        UPDATE job_card
    ...
    if new_dispatched_total is None:
        # Re-fetch to build a precise error message
        jc = await conn.fetchrow(
            "SELECT status, batch_size_kg, dispatched_to_next_kg, next_job_card_id, floor, fg_sku_name FROM job_card WHERE job_card_id = $1",
            job_card_id,
        )
        if not jc:
            return {"error": "job_card not found"}
        ...
        remaining = float(jc['batch_size_kg']) - float(jc['dispatched_to_next_kg'] or 0)
        return {"error": f"qty_kg {qty_kg} invalid. Remaining to dispatch: {remaining} kg"}

    # Fetch current JC fields we need for the rest of the flow (next_jc_id, floor, sku).
    jc = await conn.fetchrow(
        "SELECT next_job_card_id, floor, fg_sku_name, batch_size_kg FROM job_card WHERE job_card_id = $1",
        job_card_id,
    )
    ...
    if not next_jc:
        return {"error": "next job_card not found"}

    # HI-01: reject dispatch into a terminal-state next JC
    TERMINAL_NEXT_STATES = {'completed', 'closed', 'cancelled'}
    if next_jc['status'] in TERMINAL_NEXT_STATES:
        # Roll back the increment we just applied so the source JC isn't double-charged.
        await conn.execute(
    ...
    # 8. Return
    new_dispatched = float(new_dispatched_total)
    return {
