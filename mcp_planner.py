...
# ── Render keep-alive ────────────────────────────────────────────────────
# Render's free tier spins a service down after ~15 min of inactivity. A
# self-ping every 7 min keeps the dyno warm.
_KEEP_ALIVE_URL = os.environ.get(
    "KEEP_ALIVE_URL",
    "https://desktop-backend-vhf0.onrender.com/health",
)
_KEEP_ALIVE_INTERVAL_SECONDS = int(os.environ.get("KEEP_ALIVE_INTERVAL_SECONDS", str(7 * 60)))
    ...
    async with httpx.AsyncClient(timeout=10) as client:
        while True:
            try:
                ...
                logger.info("keep-alive ping %s → %s", _KEEP_ALIVE_URL, resp.status_code)
...
@mcp.tool()
async def get_planning_context(entity: str, fulfillment_ids: list[int], target_date: str = "", user_constraints_json: str = "") -> str:
    """Get full planning context: demand with BOMs, inventory, machines, capacity. Use this data to create a production schedule.

    user_constraints_json — optional JSON-encoded list of per-fulfillment constraints. Each entry:
        {
          "fulfillment_id": int,
          "allowed_floors": [str],
          "allowed_machines": {floor: [machine_name]},
          "floor_qty_kg": {floor: float},     // per-floor kg split (authoritative)
          "floor_qty_units": {floor: float}   // per-floor unit-count split (authoritative; do not recompute from kg/uom)
        }
    When supplied, the returned machines list is filtered to allowed_machines and the constraints are
    embedded into the context under "user_constraints" so the LLM can honour the floor split.
    """
    pool = await get_pool()
    t_date = target_date or str(date.today())
    user_constraints: list[dict] = []
    if user_constraints_json and user_constraints_json.strip():
        try:
            parsed = json.loads(user_constraints_json)
        except json.JSONDecodeError as e:
            raise ValueError(f"user_constraints_json is not valid JSON: {e}")
        if not isinstance(parsed, list):
            raise ValueError(
                f"user_constraints_json must be a JSON array, got {type(parsed).__name__}"
            )
    ...
    machines_list = [m for m in machines_map.values() if m['capacity']]
    if user_constraints:
        allowed_names = set()
        for c in user_constraints:
            for floor_machines in (c.get("allowed_machines") or {}).values():
                allowed_names.update(m.lower() for m in floor_machines)
        if allowed_names:
            machines_list = [m for m in machines_list if m["name"].lower() in allowed_names]

    context = {
        "date": t_date, "entity": entity, "demand": demand, "no_bom_items": no_bom,
        "inventory": inventory, "machines": machines_list,
...
async def save_production_plan(
    ...
    Each schedule item fields:
      ...
      machine_name, floor, priority (1=highest), shift (day/night),
